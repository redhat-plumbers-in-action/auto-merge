...
async function action(octokit, pr) {
    ...
    if (pr.draft || pr.currentLabels.includes(config.labels['dont-merge'])) {
        ...
        message.push(`🟢 Pull Request is not marked as draft and it's not blocked by \`${config.labels['dont-merge']}\` label`);
    }
    if (config.titlePrefix.map(prefix => pr.title.startsWith(prefix)).includes(true)) {
        err.push(`🔴 Pull Request doesn't meet requirements. The title starts with the forbidden prefix: '${config.titlePrefix}'`);
    }
    else {
    ...
    if (!config.targetBranch.includes(pr.targetBranch)) {
        ...
        err.push(`🔴 Pull Request has unsupported target branch \`${pr.targetBranch}\`, expected branches are: '${config.targetBranch}'`);
    }
    else {
